''' controller and routes for trades '''
import os
...
def index():
    return jsonify(
        status=True,
        message='Hoy to the Dockerized Flask MongoDB app! TEST'
    )
...
@application.route('/api/trades', methods=['GET'])
def list_trades():
    ''' route to get all the trades '''
    if request.method == 'GET':
        data = mongo.db.trades.find()
        return_data = list(data)
        return jsonify({'ok': True, 'trades': return_data})


@application.route('/api/importTrades', methods=['GET', 'POST'])
def post_raw_data():
...
@application.route('/api/importImages', methods=['POST'])
